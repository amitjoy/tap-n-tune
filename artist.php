...
<div class="container_16">
  ...
  <div class="grid_11 top-message">
    <h1>This is artist page list, contrary to popular belief, It has roots</h1>
  </div>
  ...
  <div class="grid_16 blog-page">
    <h1>Just Rummage</h1>
    <h2 class="blog-page-space">Music sustains to be a sole under the breath of life.</h2>
  </div>
  ...
  <!-- Filter-->
    ...
    <div id="listing">
    <?php
	$sql = mysql_query("SELECT * FROM _artist WHERE active=1");
	$i = 0;
	while($r = mysql_fetch_array($sql))
	{
	...
	{
	$sql4 = mysql_fetch_array(mysql_query("SELECT * FROM _genres WHERE idgenre = ".$r['genre']." AND active = 1"));
	}
	$genre = isset($sql3['genre_name']) ? ($sql3['genre_name']) : $sql4['genre_name'];
	$band = isset($sql2['band_name']) ? ("Band: ".$sql2['band_name']) : "Sole Artist";
	$band1 = isset($sql2['band_name']) ? $sql2['band_name'] : $r['name'];
	$sex = ($r['sex'] == 1) ? "user31" : "user11";
	$image = $r['artist_pic'] ? "admin/uploads/gallery/38x38/".$r['artist_pic']."" : "image/post/".$sex.".png";
	$sel = isset($sql2['band_name']) ? 1 : 5;
	//print_r($sql2);
	//print_r($sql3);
	echo "
	 <div class='discounted-item ".$genre." music-playlist'>		
	 <a class='picture' title='".$r['position']."' href='".$image."'>  
      <img src=".$image." alt='' class='music-playlist-left'  />
	  </a>
      <h1>".$r['name']."</h1>
      <p>".$band."</p>
		 <a href='search_pro.php?search=".$band1."&sel=".$sel."&result=b&id=tnt_search_".rand(15662,34555)."'' title='Explore Music And Video'><img src='image/theme/explore.png' alt='' class='music-playlist-right'  /></a>
	</div>
	";
	}
	?>
